...
>
> **当前视觉基线**：2026-09-21。新页面必须优先复用本文件所述的共享导航和内容卡片样式；不得恢复旧版浅色导航、页内重复标题或表格竖线。
...
- **新建单据弹窗**（如新建采购单、新建调拨单）统一以新建采购单为尺寸基准：`width: min(1500px, calc(100vw - 48px))`、`height: auto`、`max-height: min(90vh, 900px)`；禁止使用 `92vw`、`92vh` 等近乎全屏的固定尺寸。
...
- **审核类弹窗例外**：不展示单据汇总卡片或单号列表，统一遵循「1.5 审核类弹窗统一规范」。

### 1.4 弹窗表单标签布局（强制规范）
- 适用于新增、编辑等包含字段录入的业务弹窗；字段名必须固定放在对应输入组件**左侧**，标签与组件在同一行垂直居中，禁止将字段名置于组件上方。
- 以发货单新建弹窗为基准：标签宽度统一约 `100px`，标签右侧保留 `10px` 间距；同一弹窗内的标签右边界必须对齐，输入组件占用该列剩余宽度。
- 必填字段的红色星号紧贴字段名左侧；字段名、星号与输入组件不得拆到不同的行或列。
- 多行备注等跨列字段仍使用左侧标签：标签与文本域顶部对齐，文本域横向占满其余可用空间。
- 弹窗表单不使用无业务含义的分区框、分隔线、规则说明或数量汇总；仅保留标题、必要字段、明细表格和底部操作。业务确有必要的提示须经产品明确要求后才可展示。
- 本规则仅适用于弹窗表单；列表查询区继续遵循「3.9 查询区域规范」，字段名通过 placeholder 表达，不展示独立 label。

### 1.5 审核类弹窗统一规范（强制）

所有涉及「审核 / 审批 / 通过 / 驳回」的业务弹窗，必须复用同一套审核弹窗结构和视觉，不得按页面单独设计。

#### 1.5.1 结构与尺寸
- 弹窗标题统一为「审核 + 单据类型」，例如「审核调拨单」；标题下方展示已选择数量，例如「已选择 1 张待审核调拨单」。
- 审核类弹窗不展示顶部单据卡片、单号胶囊、调拨方向、数量汇总或状态列表；选择范围仅通过标题下方的数量说明表达。
- 内容区使用浅灰背景（约 `#f7f9fc`），内部使用白色审核意见卡片；卡片不使用蓝色或其他彩色边框。
- 审核意见卡片采用单列布局，标题为「审核意见」，下方放置多行文本框和右下角字数统计。
- 弹窗宽度建议为 `min(780px, calc(100vw - 48px))`，最大高度建议为 `min(82vh, 760px)`；窄屏时按视口宽度收缩。

#### 1.5.2 文案与字重
- 审核意见 placeholder 统一为：`审核通过可留空，驳回时请填写原因`。
- 不额外展示「通过可留空，驳回必填原因」等重复说明文案。
- 标题、数量说明、审核意见标题使用常规或中等字重；禁止使用过粗字体造成视觉压迫。
- 不在审核弹窗中增加与决策无关的字段或说明。

#### 1.5.3 操作按钮与校验
- 底部按钮固定为「取消」「驳回」「通过」，从左到右按该顺序排列在右侧。
- 「通过」统一使用绿色 Success 样式（建议 `#67c23a`）；「驳回」统一使用红色危险样式（建议 `#f56c6c`）；「取消」使用普通按钮样式。
- 审核通过时审核意见可为空；驳回时审核意见必填，未填写时在文本框处提示并阻止提交。
- 批量审核时，所有选中单据必须处于可审核状态；通过或驳回后统一更新状态，并将审核意见写入操作日志。
...
### 2.7 单据列表对齐、固定列与展开交互规范

- **同列字段统一左对齐**：同一列内的合并字段必须使用一致的起始边界，字段标签和值按行左对齐；不得因字段长度或字段数量不同出现居中、右对齐或上下错位。业务上明确要求路线居中的列（如调拨方向）可保留专门的对齐方式。
- **关键业务列与操作列固定**：单据列表的第一业务列（如单据号、SKU）固定在左侧，业务上需要持续对照的关联列（如调拨方向）可与其一并固定，操作列固定在右侧；横向滚动时这些列始终可见。固定列必须设置明确的层级和背景色，并复用采购单同款轻量阴影作为滚动边界：左侧固定列组右边缘使用向右阴影，右侧操作列左边缘使用向左阴影，避免与滚动内容重叠。
- **展开三角图标统一**：所有可展开的一级/二级列表必须复用项目统一的展开图标组件和样式，统一图标尺寸、颜色、间距及展开状态动画；禁止各页面自行使用不同的字符、三角形或图标实现。
- **状态标识与单号同一行**：单据号列中，状态标识必须紧跟在单号右侧，与单号保持同一行展示；状态标识统一使用状态 Tag 样式，不得单独换行到单号下方。
...
### 4.2 全局导航规范
- 顶部中台导航与左侧业务导航必须使用共享样式 `navigation.css` / `navigation.js`，保持所有页面一致；不得在单页重新定义一套导航视觉。
- 顶部导航高度 `48px`；导航颜色由当前主题统一控制，禁止页面自行覆盖导航背景、选中背景或强调色。
- 顶部固定展示且顺序为：**工作台、财务、供应链、运营、产品**。不展示「企业工单」。
- 顶部模块必须可点击切换；每项使用统一的线性图标 + 文案，最小宽度 `120px`、左右内边距 `20px`，避免模块间距过密。
- 顶部和左侧的**一级菜单**均展示统一风格图标；二级菜单只展示文字，不增加图标。
- 顶部右侧只保留「导出中心」和登录账号；**不设置全局刷新图标**。业务区内因页面功能需要保留的刷新按钮不受此规则影响。
- 工作台「业务组织」下的仓库管理与供应商列表为同级菜单。

### 4.3 主题切换规范

- 默认主题为「曜石红」：导航背景 `#202023`、选中背景 `#373739`、强调色 `#F53661`。
- 主题设置入口固定在左侧导航底部：展开状态位于收起按钮右侧；收起状态位于收起按钮上方，仅保留设置图标。
- 点击设置图标后，在侧栏右侧弹出轻量主题选择面板；面板使用主题预览色块展示当前可用主题，并以勾选标识当前主题。
- 主题预览固定顺序为：曜石红（默认）→ 云海蓝 → 极夜蓝 → 靛青灰 → 云杉绿 → 云雾紫；预览色块下不展示主题名称，主题名仅用于无障碍标签和规范文档。
- 当前支持的主题如下：

| 主题 | 顶部与左侧背景 | 选中背景 | 选中文字与右侧竖线 |
|---|---|---|---|
| 曜石红（默认） | `#202023` | `#373739` | `#F53661` |
| 极夜蓝 | `#122037` | `#1C304E` | `#86BDF5` |
| 靛青灰 | `#1F2029` | `#343640` | `#B69CFF` |
| 云海蓝 | `#FFFFFF` | `#EAF3FF` | `#1677FF` |
| 云杉绿 | `#FFFFFF` | `#EDF8F1` | `#2FA66A` |
| 云雾紫 | `#F6F7F9` | `#EAE6FF` | `#7C6EE6` |

- 每个主题切换时，必须同时更新顶部背景、左侧背景、选中文字和选中项右侧竖线；**竖线颜色必须与选中文字完全一致**。
- 云海蓝、云杉绿和云雾紫主题下，顶部与左侧导航的默认文字使用深灰，普通菜单使用中灰，导航边界使用浅灰，确保浅色导航仍有清晰层级。
- 主题选择保存在浏览器本地；刷新页面、切换模块或重新打开原型后应保持用户上一次的选择。

### 4.4 左侧业务导航与收起交互

- 左侧导航背景色与顶部一致，由当前主题统一控制；展开宽度为 `145px`，页面内容区随其同步调整。
- 一级分组标题使用白色、加粗文字；二级菜单使用浅灰文字 `#d1d1d6`。
- 选中的二级菜单背景、文字和右侧 `3px` 竖线均由当前主题控制；竖线色必须与文字强调色一致。不能使用浅蓝选中底色或底部横线。
- 菜单悬浮背景由当前主题控制；收起/展开按钮固定在左下角，并以边界线与菜单区分开。
- 点击左下角按钮后，侧栏收起为 `52px`，只保留一级菜单图标；二级菜单隐藏。
- 收起状态下，点击一级菜单图标，在其右侧弹出对应的二级菜单浮层；再次点击、点击空白处、按 `Esc` 或窗口尺寸改变时关闭浮层。
- 浮层背景、当前项和悬浮态均跟随当前主题；鼠标悬浮二级菜单时，文字使用当前主题强调色。
- 收起状态应可在本地保存，刷新页面后保持用户上次的侧栏展开/收起选择。

### 4.5 页面内容骨架与标题

- 有查询项的业务页面统一使用浅灰内容底色 `#f5f7fa`，避免查询区、操作区和列表区各自使用不一致的底色。
- 查询项区域必须使用白色圆角卡片包裹，卡片边框使用浅灰色；沿用本规范 3.9 的查询布局和交互规则。
- 查询区与列表区之间的操作按钮区域（如新建、导入、导出、批量操作）直接放在灰色内容底上，**不使用白色卡片包裹**。
- 列表（包括状态 Tab 与表格）使用独立白色圆角卡片包裹；分页、表格主体与状态栏应作为同一列表卡片的连续区域，避免重复套框。
- 页面没有业务 Tab 时，不在内容区重复展示页面名称（不显示单独 h1、page-title 或伪 Tab 标题）。
- 只有页面确有可切换的业务 Tab 时才展示 Tab 栏；Tab 是交互入口，不能仅为展示页面名称而保留。

### 4.6 列表与表格视觉规范

- 所有列表表格仅保留**横向行分隔线**，移除列与列之间的竖向边框；表格外层卡片的边框可保留。
- 此规则同时适用于原生 `table`、Element Plus `el-table`、嵌套表格和固定列；固定列不得使用实体竖向分隔线，但允许复用采购单同款轻量边界阴影，不得使用粗重或页面自定义的阴影样式。
- 表头与表体的横线颜色保持克制、统一，不能因移除竖线而取消行分隔或降低信息可读性。
- 页面内部的功能工具栏（例如列表上方的导出、刷新）按业务需要保留；不得与顶部全局导航入口混淆。

#### 4.6.1 单据状态栏规范（以发货单为唯一基准）

- 适用于具有单据生命周期状态的列表页，包括发货单、调拨单、采购单等；状态栏必须复用发货单页面（`pages/shipment-orders/index.html`）的视觉规格，禁止单页自行调整高度、颜色或数量气泡样式。
- 状态栏与表格属于同一张白色列表卡片的连续区域：状态栏外层总高度为 `50px`（含上下 `1px` 边框），左右内边距为 `16px`；其中状态 Tab 内容高度统一为 `48px`，不得额外留白或使用按钮化外框。
- 未选中 Tab 使用深灰文字；当前选中 Tab 的文字与底部 `2px` 指示线统一使用发货单蓝色 `#409EFF`。
- 状态数量使用圆角气泡：最小宽度 `22px`、高度 `18px`、水平内边距 `6px`、左间距 `6px`、圆角 `10px`、默认背景 `#F2F4F7`；当前选中状态的气泡背景为 `#ECF5FF`，文字为 `#409EFF`。
- 状态栏底部不额外绘制分隔线；由表格表头的边界线承接，保持状态栏、表格主体和分页区为连续卡片。

#### 4.6.2 下拉选择组件规范（以发货单为唯一基准）

- 所有页面的查询项、弹窗表单和页码条数选择，统一使用与发货单 `el-select` 一致的输入框式下拉组件；不得直接展示浏览器原生 `<select>` 的系统菜单。
- 触发器使用白底、浅灰边框、右侧箭头；展开菜单使用白底、浅灰描边和阴影。选中项使用浅蓝底 `#ECF5FF`、蓝色文字 `#409EFF` 和勾选标识；悬浮项使用浅灰底。
- 下拉组件必须保留原有的取值、筛选、联动和 `change` 事件，不得因视觉替换影响业务交互；动态创建的弹窗字段也必须自动套用同一组件。
- 已使用 Element Plus `el-select` 或已符合本条视觉和交互规格的组件无需重复改造；其余原生下拉必须复用共享组件，禁止页面自行实现不同的菜单样式。
...
- 采购单、发货单、调拨单及后续新增单据列表必须加载根目录 `document-list.css`，并在主表格或列表容器上使用 `document-list` 类；页面级 CSS 只负责列宽、业务字段和状态差异。
- 合并字段统一使用 `.field-line`，字段名使用 `.field-label`，字段值使用 `.field-value`；字段名和值均左对齐、常规字重，不在页面内重复定义字体、字号、颜色和行高。
- 展开控件统一复用共享展开组件样式，使用 `.document-expand`（存量页面的 `.expand-row` / `.expand-btn` 由共享样式兼容）；禁止使用文字字符、自绘三角形或单页独立箭头实现另一套样式。
- 列表公共视觉 Token 统一维护在 `document-list.css`：字体族为 Arial / 苹方 / Microsoft YaHei，字段标签色 `#8F959E`，字段值色 `#606266`，合并字段字号 `12px`，调拨方向等主展示值字号 `14px`。
- 新增单据页面必须先复用共享列表样式，再添加业务列配置；如确需例外，必须在本规范中记录原因、适用范围和回归页面，避免同类单据再次产生视觉分叉。
...
- [ ] **新建单据弹窗尺寸（强制必查）**：是否使用新建采购单基准 `min(1500px, calc(100vw - 48px))`，高度自适应且最大不超过 `min(90vh, 900px)`？
- [ ] 固定列是否正常工作？输入框能否不滚动直接看到？
- [ ] 固定列是否复用采购单同款边界阴影（左侧固定列组右边缘向右阴影、右侧操作列左边缘向左阴影）？
- [ ] 合并列中的字段名是否保持了原名？
- [ ] 子件格式是否带 `×配比`？
- [ ] 批量弹窗有无汇总栏？单条弹窗标题是否含单据号？
- [ ] **审核弹窗（强制必查）**：是否复用统一审核结构？是否移除顶部单据卡片和单号胶囊？审核意见卡片是否为白底无彩色边框？通过按钮是否为绿色、驳回按钮是否为红色？驳回是否强制填写原因？
- [ ] **弹窗表单（强制必查）**：字段名是否全部位于组件左侧、同一列标签是否右边界对齐？是否未将字段名放在组件上方，且未加入无业务含义的分区线、提示文案或汇总？
- [ ] **下拉组件（强制必查）**：查询区、弹窗和页码条数选择是否均为发货单同款输入框式下拉？展开菜单是否没有浏览器原生样式，且选择后原有筛选/联动仍生效？
- [ ] 有无「一键填满」和「清空」按钮？
...
- [ ] **全局导航（强制必查）**：顶部是否只显示工作台、财务、供应链、运营、产品？是否已移除企业工单和全局刷新图标？顶部及左侧一级菜单是否都有统一图标？
- [ ] **主题切换（强制必查）**：左侧底部是否有主题设置图标？是否可选择曜石红、极夜蓝、靛青灰、云海蓝、云杉绿、云雾紫？切换后顶部和左侧背景、选中文字及右侧竖线是否同步更新，且文字与竖线颜色一致？刷新后是否保留选择？
- [ ] **左侧导航（强制必查）**：展开宽度是否为 145px？选中项是否使用当前主题的选中背景、强调文字和同色右侧竖线？收起后是否为 52px 图标栏，并可弹出可点击的二级菜单？
- [ ] **页面骨架（强制必查）**：无业务 Tab 的页面是否未重复展示页面名称？查询区和列表区是否分别使用白色卡片，操作按钮区是否未套卡片？
- [ ] **列表边框（强制必查）**：是否已移除所有竖向分隔线（含 Element Plus 固定列和嵌套表格），同时保留横向行分隔线？
