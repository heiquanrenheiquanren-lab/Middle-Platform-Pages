...
- **第一业务列与操作列固定**：单据列表的第一业务列（如单据号、SKU）固定在左侧，操作列固定在右侧；横向滚动时两列始终可见。固定列必须设置明确的层级、背景色和边界处理，避免与滚动内容重叠。
...
- 此规则同时适用于原生 `table`、Element Plus `el-table`、嵌套表格和固定列；固定列不得保留竖向阴影或分隔条。
