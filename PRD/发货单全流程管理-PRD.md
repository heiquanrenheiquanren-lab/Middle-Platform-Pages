...
3. 对已接入易仓/海外仓 WMS 的目的仓，系统可根据收货上架回传自动生成收货结果和收货批次；接口未接入或数据未同步时，用户可点击「刷新」触发再次拉取。刷新必须幂等，不得重复累计收货。
4. 未接入易仓的速卖通仓、美客多仓等平台仓不承诺自动收货；是否允许继续使用人工收货入口沿用现有收货能力，具体仓库范围待确认。
5. 少收原因沿用既有收货功能，不在本次新增设计范围内。
6. 待收货、部分收货支持手动完成：完结原因必填，枚举为货物丢失、货物损坏、业务取消、其他；备注选填，最长 300 字；未收数量保留为收发差异且不改记为已收数量。
7. 手动完成只改变中台发货单状态为已完成，不自动把第三方系统单据标记为已完成；是否同步触发易仓/海外仓作废或结单接口，见 9.1 和待确认问题。
...
| P0：已提货或已发货后的“结单/异常撤单”是否纳入 1.0；允许哪些状态；是否需要同步易仓/海外仓作废和库存释放？ | 2.4、6.8、9.1；影响状态机、库存、费用、接口、权限和审计 | 业务负责人、库存/仓储负责人、研发负责人 |
...
| P1：已按评审确认「编辑移除释放回待发货、取消/作废计划同步作废」；后续结单/异常撤单场景下的计划逆向状态是否沿用同规则？ | 6.8、9.1；影响计划数据和逆向流程 | 业务负责人、计划负责人 |
