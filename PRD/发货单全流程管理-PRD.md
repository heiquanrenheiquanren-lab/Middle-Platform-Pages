...
4. 加工扣减口径：仅按子 SKU 的**在库量**计算，**不含**「在库（待发）量」和「在途／在产量」。
5. **加工数量上限**：`min(提货数量−已加工数量，各子 SKU 在库量÷对应 BOM 配比)`。即由在库量最少的子件卡上限（木桶短板）；即使其他子件在库更多，本次加工数量也不得超过该组合 SKU 的提货数量；提交时按此上限校验。
...
- [ ] 加工数量上限 = min(提货数量−已加工数量，子件在库量÷BOM 配比)，超过提货数量的输入被阻止。

- [ ] 加工可用量按子 SKU 在库量和 BOM 配比校验，成功后生成加工批次和子件消耗记录。
