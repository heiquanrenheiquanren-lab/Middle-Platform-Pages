...
发货单提货成功后，必须触发现有供应商库存提货批次回写逻辑；团队库存及中台库存与易仓、领星等外部系统之间的交互，沿用现有库存流转规则和接口能力。开发时需复用现有逻辑，保证库存、提货批次、外部系统数据的一致性、幂等性和可追溯性。本次不重新定义库存流转规则。
...
- 异仓/海外仓收货数据的自动拉取与手动刷新；未接入异仓的第三方平台仓不承诺自动收货。
...
3. 提货完成后，只有所有 SKU 均完成最终确认，单据才根据是否存在未加工完成的组合 SKU 进入「可加工」或「待发货」。
...
5. 当前设计口径：平台为亚马逊的货物视为 FBA；FBA 货物不可与非 FBA 货物合并至同一张发货单。FBA 的最终判定字段和例外场景待确认。
...
2. 支持从单张发货单操作，也支持对可加工状态发货单批量操作；批量提交须按单据分别校验并保证单据级幂等。
...
3. 对已接入异仓/海外仓 WMS 的目的仓，系统可根据收货上架回传自动生成收货结果和收货批次；接口未接入或数据未同步时，用户可点击「刷新」触发再次拉取。刷新必须幂等，不得重复累计收货。
4. 未接入异仓的速卖通仓、美客多仓等平台仓不承诺自动收货；是否允许继续使用人工收货入口沿用现有收货能力，具体仓库范围待确认。
5. 少收原因沿用既有收货功能，不在本次新增设计范围内。
6. 待收货、部分收货支持手动完成：完结原因必填，枚举为货物丢失、货物损坏、业务取消、其他；备注选填，最长 300 字；未收数量保留为收发差异且不改记为已收数量。
7. 手动完成只改变中台发货单状态为已完成，不自动把第三方系统单据标记为已完成；是否同步触发异仓/海外仓作废或结单接口，见 9.1 和待确认问题。
...
| 异仓 / 海外仓 WMS | 发货推送、收货回传、手动刷新、自动收货批次 | 依赖接口；本 PRD 定义中台侧处理 | 接口字段、回传时点、失败重试和结单接口/RPA |
...
| 问题 | 影响范围 | 建议确认对象 |
|---|---|---|
| P0：FBA 发货单是由中台新建，还是由领星拉取后关联中台计划；领星与中台的拉取/推送时点、去重键和失败补偿是什么？ | 4.4、6.2、9.1；影响创建入口、接口、状态和验收 | 业务负责人、领星/研发负责人 |
| P0：已提货或已发货后的“结单/异常撤单”是否纳入 1.0；允许哪些状态；是否需要同步异仓/海外仓作废和库存释放？ | 2.4、6.8、9.1；影响状态机、库存、费用、接口、权限和审计 | 业务负责人、库存/仓储负责人、研发负责人 |
| P0：各动作对应的具体角色、审批关系和团队/仓库数据范围是什么？ | 9.4；影响页面权限、接口鉴权、导出和验收 | 业务负责人、权限平台主管 |
| P0：提货数量在「申报 < 计划」场景下是否可以超过申报数量，仍以计划数量为上限？ | 4.3、6.3、7；影响数量校验、差异口径和测试 | 业务负责人 |
| P1：FBA 的判定字段是什么；是否允许同一平台下按仓库/团队再细分 FBA 与非 FBA？ | 3.1、6.1、6.2；影响混单校验和筛选 | 业务负责人、数据负责人 |
| P1：自动收货覆盖哪些仓库；WMS 回传字段、回传时点、外部单号/批次号和失败重试规则是什么？未接入平台仓是否允许人工收货？ | 6.6、8、9.1；影响接口、状态和验收 | 仓储/研发负责人 |
| P1：箱规字段从哪里取；整箱数量是否允许手动调整；手动调整是否需要审批？ | 6.9、8、十；影响页面字段、接口、箱信息和审计 | 物流团队、业务负责人 |
| P1：少提、少发、少收以及“其他”原因的最终枚举值和备注长度是否统一？ | 6.3、6.5、6.6、验收 | 业务负责人、测试负责人 |
| P1：数量调整后是否允许状态回退到待提货；若允许，发货计划关联关系是否保持不变？ | 6.7、计划联动、状态验收 | 业务负责人、研发负责人 |
| P1：取消发货后的发货计划状态是已作废、恢复待发货，还是需要其他状态；草稿作废是否始终不回写？ | 6.8、9.1；影响计划数据和逆向流程 | 业务负责人、计划负责人 |
| P2：导出是否需要按团队、仓库、组织等维度控制数据范围；导出是否采用同步下载还是异步任务？ | 6.10、9.4；影响权限、性能和验收 | 权限平台主管、研发负责人 |
