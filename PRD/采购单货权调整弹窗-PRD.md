...
| 调入平台 | 必填 |
| 调入店铺 | 必填，根据调入平台筛选 |
| 应用到勾选 SKU | 将平台、店铺应用到已勾选行；未勾选时不执行 |
...
|---|---|
| 调入平台 / 店铺 | 目标平台和店铺 |
| 调入 SellerSKU / ASIN | 沿用现有 SKU 映射逻辑 |
| 调入团队 | 根据调入店铺自动带出，不允许人工修改 |
...
4. 成功：更新采购单和供应商库存数据，提示成功，关闭弹窗并刷新采购单页面。
5. 失败：保留弹窗内容，提示原因并允许重试。
...
flowchart TD
    ...
    E --> F[选择调入平台和店铺]
    F --> G[根据调入店铺自动带出目标团队]
    G --> H[填写目标 SellerSKU/ASIN和调入数量]
    H --> I{校验是否通过?}
    I -- 否 --> J[提示错误，返回修改]
    J --> H
    I -- 是 --> K[二次确认提交]
    K --> L{后端校验状态、权限和最新在途数量}
    L -- 不通过 --> M[提示失败，保留内容]
    L -- 通过 --> N[更新采购单原团队和目标团队数据]
    N --> O[同步更新供应商库存原团队和目标团队数据]
    O --> P{两边是否全部成功?}
    P -- 否 --> Q[整体回滚，不产生部分调整]
    P -- 是 --> R[保留原团队行，刷新两侧数据]
    R --> S[提示货权调整成功]
