...
本期只同步并展示 FBA 货件列表返回的货件状态、申报量和签收量，不接入外部发货执行、配货、物流及费用数据。库存实际扣减、头程在途和后续团队 / 平台仓库存更新继续沿用现有库存逻辑，不在本 PRD 中新增触发规则。
...
| 中台库存 | 货件状态和数量同步后的库存衔接沿用现有逻辑，不新增本次专属流转规则。 | 部分，归属口径待确认 |
